...
gchar *text_from_size(int size)
{
        ...
        *d = '\0';
		...
		if ( multiple <= 0 ) sprintf( submit_string, "%d b", submit );
		if ( multiple == 1 ) sprintf( submit_string, "%d kb", submit );
		if ( multiple == 2 ) sprintf( submit_string, "%d mb", submit );
		if ( multiple == 3 ) sprintf( submit_string, "%d gb", submit );
		if ( multiple >= 4 ) sprintf( submit_string, "%d tb", submit );
